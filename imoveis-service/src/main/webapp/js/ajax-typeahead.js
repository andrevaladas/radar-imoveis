...
/*
 * 
 * Modifications by Paul Warelis, André Valadas
 * 
 */
...
!function($){
	...
	var Typeahead = function (element, options) {
		...
		if (this.options.ajax) {
			...
			this.ajax = {
				...
				idProperty : ajax.idProperty || null,
				displayField : ajax.displayField || null,
				sorterField : ajax.sorterField || null,
	...
	Typeahead.prototype = {
		...
		select: function () {
			...
			if (this.options.ajax) {
				var ajax = this.options.ajax;
				if (ajax.idProperty && ajax.displayField) {
					var elementId = $('#'+ajax.idProperty);
					if (elementId) {
						var $idItem = $.grep(this.dataCache, function (item) {
							var field = item[ajax.displayField];
							if (field == val) {
								return item;
							}
						})
						elementId.val($idItem[0]['id']);
					}					
				}
			}
			return this.hide()
		...
		sorter: function (items) {
			...
			while (item = items.shift()) {
				if (this.ajax) {
					if (this.ajax.sorterField) {
						item = item[this.ajax.sorterField]
					} else if (this.ajax && this.ajax.displayField) {
			...
			return beginswith.concat(caseSensitive, caseInsensitive).sort();
		},
